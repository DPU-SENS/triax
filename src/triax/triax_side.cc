...
DEFUN_DLD (triax_side, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{ret} = } triax_side (@var{triax}, @var{width})\n \
\n\
Check or set the side slit width.\n \
\n\
@var{triax} - instance of @var{octave_triax} class.@*\
@var{width} - Desired slit width (microns).\n \
\n\
Shall return the current side slit width in microns.\n \
@end deftypefn")
{
    if (!type_loaded) {
    ...
    if (slit_width >= 0) {
        ret_val = octave_value(triax->set_side_width(slit_width));
    } else {
        ret_val = octave_value(triax->get_side_width());
    }
