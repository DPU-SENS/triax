...
DEFUN_DLD (triax_turret, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{ret} = } triax_turret (@var{triax}, @var{pos})\n \
\n\
Check or set the turret position (nm).\n \
\n\
@var{triax} - instance of @var{octave_triax} class.@*\
@var{pos} - Desired turret position (nm).\n \
\n\
Shall return the current turret position in nm.\n \
@end deftypefn")
{
    if (!type_loaded) {
    ...
    if (pos >= 0) {
        ret_val = octave_value(triax->set_turret_pos(pos));
    } else {
        ret_val = octave_value(triax->get_turret_pos());
    }
